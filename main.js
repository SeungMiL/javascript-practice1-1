...
let sentence = ["1잔만 마셔라!", "2잔 마셔라!!", "3잔 마셔라!!!", "뒤질때까지 마셔라!!!!!!!!"]

chanceArea.innerHTML = `남은 기회:${chances}`;
playButton.addEventListener("click", play);
resetButton.addEventListener("click", reset);
userInput.addEventListener("focus", focusInput);
// alcohol.textContent = "죽을떄 까지 마셔라!!!";
...
function play() {
  ...
  if (gameOver == true) {
    ...
    alcohol.style.display=("block");
    alcohol.textContent = sentence[(Math.floor(Math.random() * sentence.length))];
  }
}
